...
export function displayDataDOMProject()
{
    ...
    for(let IndexNumber = 0; IndexNumber < arrayDataProject.length; IndexNumber++){
        parentElementProject.appendChild(createDOMProjectElements(arrayDataProject[IndexNumber]));
    }    
}
...
function setDOMProjectElementsData({parentElementCurrentProject,projectTitle,projectObject,btnDelete}){
    ...
    // projectTitle.addEventListener('click',()=>{console.log("REPLACE WITH FUNCTION")}) //replace with function
